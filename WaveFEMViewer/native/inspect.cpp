...
int main(int argc, char* argv[]) {
    if (argc < 2 || argc > 3) {
        std::cerr << "usage: wavefem-viewer-inspect FILE [RESULT_INDEX]\n";
        return 2;
    }
    try {
        const auto resultIndex = argc == 3 ? static_cast<std::size_t>(std::stoull(argv[2])) : 0U;
        const auto started = std::chrono::steady_clock::now();
        const auto index = wavefem::H5Reader::loadIndex(std::filesystem::path(argv[1]));
        const auto indexed = std::chrono::steady_clock::now();
        const auto result = wavefem::H5Reader::loadResult(*index, resultIndex);
