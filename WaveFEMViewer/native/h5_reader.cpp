...
std::shared_ptr<FileIndex> H5Reader::loadIndex(const std::filesystem::path& path) {
    ...
    if (readStringAttribute(file.get(), "format") != "wavefem") {
        fail("The selected file is not a WaveFEM HDF5 result.");
    }
    if (readIntegerAttribute(file.get(), "schema_version") != 1) {
        fail("This viewer supports WaveFEM HDF5 schema version 1 only.");
    }
