...
solver = Periodic_3D_Mode_Solver(Nx=Nx, Ny=Ny, Nz=Nz,
                                 x_range=6.6e-3, y_range=6e-3, z_range=5.7e-3,
                                 freq=22e9, num_modes=2, tol=1e-2)
# Build waveguide unit cell
solver.add_object(1e8, 1, slice(0, Nx), slice(Ny - 16, Ny - 15), slice(0, Nz))
solver.add_object(1, 1, slice(2, Nx - 2), slice(Ny - 16, Ny - 15), slice(0, 7))
solver.add_object(3, 1, slice(0, Nx), slice(Ny - 15, Ny - 7), slice(0, Nz))
...
solver.add_absorbing_boundary(['+y'], width=10, max_loss=15, n=3)
