...
if ([System.IO.Path]::GetFileName($destinationRoot) -ne 'FEMPeriodicViewer') {
    throw "Destination directory must be named FEMPeriodicViewer: $destinationRoot"
}
...
$stagingRoot = [System.IO.Path]::GetFullPath(
    (Join-Path $destinationParent ".FEMPeriodicViewer-stage-$PID")
)
$backupRoot = [System.IO.Path]::GetFullPath(
    (Join-Path $destinationParent ".FEMPeriodicViewer-backup-$PID")
)
...
foreach ($temporaryRoot in ($stagingRoot, $backupRoot)) {
    ...
    if (-not $leaf.StartsWith('.FEMPeriodicViewer-', [System.StringComparison]::Ordinal)) {
...
try {
  ...
  & $cmake --fresh -S $sourceRoot -B $buildRoot -G Ninja `
    ...
    "-DFEM_PERIODIC_VIEWER_WITH_VTK=$vtkSetting"
...
$viewer = Join-Path $binDirectory 'fem-periodic-viewer.exe'
$inspector = Join-Path $binDirectory 'fem-periodic-inspect.exe'
...
Write-Host "Run: $(Join-Path $destinationRoot 'bin\fem-periodic-viewer.exe') [result.h5]"
