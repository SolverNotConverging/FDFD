...
int runInspector(const std::vector<std::filesystem::path>& arguments) {
    if (arguments.size() < 2 || arguments.size() > 5) {
        std::cerr << "usage: fem-periodic-inspect FILE [CASE_INDEX] [MODE_INDEX] [--coefficients]\n";
        return EXIT_FAILURE;
    }
    try {
        ...
        std::cerr << "fem-periodic-inspect: " << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
...
int main() {
    ...
    if (wideArguments == nullptr) {
        std::cerr << "fem-periodic-inspect: could not decode the Windows command line.\n";
        return EXIT_FAILURE;
    }
