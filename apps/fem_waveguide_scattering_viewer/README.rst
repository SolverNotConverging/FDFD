...
The reader accepts ``format=cem-fem-results``, ``schema=1.0``,
``solver_family=waveguide_scattering``, ``dimension=2``, ``units=SI``,
and ``time_convention=exp(+i*omega*t)``. The spatial field representation is
``sampled-fields; exp(-i*ky*y)``. Both single results and frequency sweeps
...
Run standalone build and installer commands below from
``apps/fem_waveguide_scattering_viewer``. The build requires:
...
The GUI is produced as ``build/fem-waveguide-scattering-viewer.app``; the inspect utility is a regular Mach-O executable in ``build/``.
...
* ``fem-waveguide-scattering-viewer`` — the Qt GUI (``.exe`` on Windows or ``.app`` on macOS);
...
.. code-block:: powershell
   ...
   powershell -ExecutionPolicy Bypass -File .\scripts\install.ps1 `
     -Destination D:\Tools\FEMWaveguideScatteringViewer
...
From Python, call ``result.show()`` or ``sweep.show()``. These operations use
temporary HDF5 archives and clean up when the native viewer exits.
``show(block=False)`` returns after launching. Saving is explicit with
``result.save(path)``. Set ``FEM_WAVEGUIDE_SCATTERING_VIEWER_EXECUTABLE`` to
override executable discovery. The ``fem-waveguide-scattering-inspect --gui``
command opens the current directory in the native viewer.
...
.. code-block:: powershell
   ...
   & "$env:LOCALAPPDATA\FEMWaveguideScatteringViewer\bin\fem-waveguide-scattering-viewer-inspect.exe" `
     D:\results\device_sweep.h5
...
.. code-block:: powershell
   ...
   fem-waveguide-scattering-viewer-inspect.exe D:\results\device_sweep.h5 4
...
For a custom location, remove that installation directory directly or pass a directory named ``FEMWaveguideScatteringViewer`` to the script. The uninstall script refuses to recursively remove a directory with any other final name.
