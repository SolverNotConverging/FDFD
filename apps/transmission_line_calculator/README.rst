...
.. code-block:: console
   ...
   cmake -S apps/transmission_line_calculator -B build -DTL_BUILD_EXAMPLES=ON
...
With a standalone build these are under ``build/`` (or a configuration subdirectory for Visual Studio). A repository-root build places them under ``build/apps/transmission_line_calculator/``.
