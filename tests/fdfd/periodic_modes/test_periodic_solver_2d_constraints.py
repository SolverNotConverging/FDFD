...
from fdfd_periodic_modes.solver_2d import _PeriodicModeSolver2D as PeriodicModeSolver2D


periodic_solver_2d_module = importlib.import_module(
    "fdfd_periodic_modes.solver_2d"
)
...
class PeriodicModeSolver2DConstraintTests(unittest.TestCase):
    ...
    def test_refined_solve_uses_shared_backend_and_restores_exact_zeros(self):
        ...
        def fake_refined(
            ...
            *,
            ...
            return SimpleNamespace(
                ...
                eigenvectors=vector.astype(complex),
                physical_residuals=np.array([1e-9]),
                restart_count=2,
            )
        ...
        self.assertEqual(captured["A"].shape, (reduced_size, reduced_size))
        self.assertEqual(captured["B"].shape, (reduced_size, reduced_size))
        self.assertEqual(captured["backend"], "python")
