...
def test_material_and_internal_pec_automatic_mesh_refinement() -> None:
    ...
    solver.add_rectangle(x_range=(0.0, 1.27 * mm), z_range=(0.0, 8.0 * mm), name='grounded_dielectric_slab', material=materials.Material(epsilon=10.2, mu=1.0))
...
def test_uniform_pec_tm_plane_wave_and_reconstructed_fields() -> None:
    solver = PeriodicModeSolver2D(frequency=10000000000.0, x_range=(0.0, 0.004), z_range=(0.0, 0.005), polarization='TM', boundary=materials.PEC, background_material=materials.Material(epsilon=2.25, mu=1.0))
...
def test_geometry_change_invalidates_discretization() -> None:
    ...
    solver.add_rectangle(x_range=(0.005, 0.008), z_range=(0.001, 0.002), material=materials.Material(epsilon=2.0, mu=1.0))
    with pytest.raises(NotDiscretizedError):
        solver._solve_once()
...
def test_periodic_seam_allows_a_material_interface_when_topology_matches() -> None:
    ...
    matching.add_rectangle(x_range=(0.005, 0.01), z_range=(0.0, 0.001), material=materials.Material(epsilon=2.0, mu=1.0))
    matching.add_rectangle(x_range=(0.005, 0.01), z_range=(0.004, 0.005), material=materials.Material(epsilon=2.0, mu=1.0))
    ...
    mismatched.add_rectangle(x_range=(0.005, 0.01), z_range=(0.0, 0.001), material=materials.Material(epsilon=2.0, mu=1.0))
    mismatched.add_rectangle(x_range=(0.005, 0.01), z_range=(0.004, 0.005), material=materials.Material(epsilon=3.0, mu=1.0))
...
def test_lossy_longitudinal_bilayer_matches_transfer_matrix() -> None:
    ...
    solver = PeriodicModeSolver2D(frequency=frequency, x_range=(0.0, 0.004), z_range=(0.0, period), polarization='TE', boundary=materials.PMC, background_material=materials.Material(epsilon=epsilon_1, mu=1.0))
    solver.add_rectangle(x_range=(0.0, 0.004), z_range=(thickness, period), material=materials.Material(epsilon=epsilon_2, mu=1.0))
...
def test_transverse_pml_has_passive_sign_and_converges() -> None:
    ...
    for maximum in (2.0e-3, 1.0e-3):
        solver = PeriodicModeSolver2D(frequency=10000000000.0, x_range=(0.0, 0.02), z_range=(0.0, 0.005), polarization='TE', background_material=materials.Material(epsilon=2.25, mu=1.0))
...
def test_uniform_fem_and_fdfd_agree() -> None:
    from fdfd_periodic_modes.solver_2d import (
        _PeriodicModeSolver2D as FDFDModeSolver2D,
    )
    ...
    fem = PeriodicModeSolver2D(frequency=frequency, x_range=(0.0, width), z_range=(0.0, period), polarization='TE', boundary=materials.PMC, background_material=materials.Material(epsilon=epsilon, mu=1.0))
