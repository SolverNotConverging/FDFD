...
def test_geometry_is_placed_before_explicit_discretization() -> None:
    ...
    region = solver.add_rectangle(x_range=(0.25, 0.75), y_range=(0.1, 0.4), name='core', material=materials.Material(epsilon=2.25, mu=1.0))
    ...
    solver.add_circle(center=(0.5, 0.25), name='inclusion', material=materials.Material(epsilon=1.5, mu=1.0), radius=0.04)
    assert not solver.discretized
...
def test_homogeneous_pec_te10_matches_the_rectangular_guide_limit() -> None:
    ...
    expected = np.sqrt(0.75)
    solver = ModeSolver2D(frequency=C_0, x_range=1.0, y_range=0.5, boundary=materials.PEC)
...
def test_passive_bulk_loss_uses_negative_imaginary_neff() -> None:
    ...
    solver = ModeSolver2D(frequency=C_0, x_range=1.0, y_range=0.5, background_material=materials.Material(epsilon=epsilon, mu=1.0))
...
def test_passive_sibc_post_uses_negative_imaginary_neff() -> None:
    ...
    solver = ModeSolver2D(frequency=C_0, x_range=1.0, y_range=0.5, boundary=materials.PEC)
    post = solver.add_geometry(name='lossy_post', shape=shapes.Rectangle(bounds=((0.56, 0.68), (0.11, 0.23))), material=SurfaceImpedance(impedance=20.0 + 20j))
    solver.mesh(resolution=(12, 6))

    assert post.material.impedance == 20.0 + 20.0j
    assert solver.mesh_data.boundary_facets[post.name].size > 0
    assert solver.system.impedance_boundaries[0][1] == post.material.impedance
