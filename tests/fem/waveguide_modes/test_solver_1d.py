...
def test_pml_quadrature_converges_to_integrated_weak_form() -> None:
    ...
    solver = ModeSolver1D(frequency=c, x_range=(0.0, 1.0))
    solver.add_pml(thickness=0.4, order=3, sigma_max=5.0, direction='x+')
    solver.mesh(resolution=8, wavelength_elements=4, material_aware=False)
    ...
    for order in (2, 12):
        ...
        vector = solver.mesh_data.nodes[system.free_nodes]
...
def test_discretization_is_explicit_and_conforms_to_layer_interfaces() -> None:
    solver = ModeSolver1D(frequency=30000000000.0, x_range=0.015)
    solver.add_layer(x_range=(0.0037, 0.0113), name='core', material=materials.Material(epsilon=4.0, mu=1.0))

    with pytest.raises(NotDiscretizedError, match="mesh"):
        solver._solve_once(num_modes=2)

    mesh = solver.mesh(resolution=31)
...
def test_material_aware_mesh_is_denser_in_high_dk_layer() -> None:
    solver = ModeSolver1D(frequency=c, x_range=(0.0, 1.0))
    solver.add_layer(x_range=(0.5, 1.0), name='high_dk', material=materials.Material(epsilon=9.0, mu=1.0))

    mesh = solver.mesh(resolution=20, wavelength_elements=4, material_aware=True)
...
def test_material_aware_can_be_disabled_without_violating_shortest_wavelength() -> None:
    solver = ModeSolver1D(frequency=c, x_range=(0.0, 1.0))
    solver.add_layer(x_range=(0.5, 1.0), material=materials.Material(epsilon=9.0, mu=1.0))

    mesh = solver.mesh(resolution=20, wavelength_elements=4, material_aware=False)
    widths = np.diff(mesh.nodes)
...
def test_material_aware_mesh_grades_index_contrast_below_unity() -> None:
    solver = ModeSolver1D(frequency=1000000.0, x_range=(0.0, 1.0), background_material=materials.Material(epsilon=0.25, mu=1.0))
    solver.add_layer(x_range=(0.5, 1.0), name='higher_index', material=materials.Material(epsilon=1.0, mu=1.0))
    mesh = solver.mesh(resolution=20, wavelength_elements=4)
    centres = 0.5 * (mesh.nodes[:-1] + mesh.nodes[1:])
    widths = np.diff(mesh.nodes)
...
def test_anisotropic_wavenumber_bound_uses_largest_epsilon_and_mu_axes() -> None:
    solver = ModeSolver1D(frequency=c, x_range=(0.0, 0.1), background_material=materials.Material(epsilon=(100.0, 1.0, 1.0), mu=(1.0, 100.0, 1.0)))
    mesh = solver.mesh(resolution=2, wavelength_elements=4)
...
def test_wavelength_elements_caps_size_in_homogeneous_high_dk_medium() -> None:
    solver = ModeSolver1D(frequency=c, x_range=(0.0, 1.0), background_material=materials.Material(epsilon=9.0, mu=1.0))
    mesh = solver.mesh(resolution=2, wavelength_elements=12)
...
def test_refine_remeshes_and_invalidates_previous_modes() -> None:
    solver = ModeSolver1D(frequency=20000000000.0, x_range=0.02)
    solver.add_layer(x_range=(0.007, 0.013), material=materials.Material(epsilon=4.0, mu=1.0))
    coarse = solver.mesh(resolution=24, wavelength_elements=6)
    solver.solve(max_refinements=0, num_modes=1)
    assert solver.result is not None

    fine = solver.refine(factor=2.0)

    assert fine is solver.mesh_data
...
def test_refine_requires_a_current_mesh_and_density_increase() -> None:
    solver = ModeSolver1D(frequency=10000000000.0, x_range=0.01)
    with pytest.raises(NotDiscretizedError, match="mesh"):
        solver.refine()

    solver.mesh(resolution=12)
...
def test_failed_rediscretization_keeps_previous_mesh_settings_atomic() -> None:
    solver = ModeSolver1D(frequency=10000000000.0, x_range=0.01)
    original = solver.mesh(resolution=12, quadrature_order=4)

    with pytest.raises(MeshError):
        solver.mesh(resolution=1, quadrature_order=8)

    assert solver.mesh_data is original
...
def test_geometry_placement_after_meshing_invalidates_the_mesh() -> None:
    solver = ModeSolver1D(frequency=25000000000.0, x_range=0.012)
    solver.mesh(resolution=30)
    solver.add_layer(x_range=(0.004, 0.008), material=materials.Material(epsilon=2.25, mu=1.0))
...
def test_homogeneous_pec_parallel_plate_spectrum_matches_analytic_modes() -> None:
    frequency = 20e9
    width = 20e-3
    solver = ModeSolver1D(frequency=frequency, x_range=width)
    solver.mesh(resolution=100)
    modes = solver.solve(max_refinements=0, residual_tolerance=1e-09, num_modes=3)
...
def test_sparse_shift_invert_retains_degenerate_te_tm_pair() -> None:
    ...
    width = 20e-3
    solver = ModeSolver1D(frequency=frequency, x_range=width)
    solver.mesh(resolution=220)
    modes = solver.solve(max_refinements=0, dense_limit=50, residual_tolerance=1e-08, num_modes=3, neff_guess=1.0)
...
def test_uniform_diagonal_tm_tem_mode_uses_correct_tensor_components() -> None:
    ...
    solver = ModeSolver1D(frequency=15000000000.0, x_range=0.01, background_material=materials.Material(epsilon=epsilon, mu=mu))
    solver.mesh(resolution=40)
    mode = solver.solve(max_refinements=0, num_modes=1, neff_guess=expected)[0]
...
def test_pmc_outer_walls_produce_the_dual_te_tem_mode() -> None:
    ...
    solver = ModeSolver1D(frequency=12000000000.0, x_range=0.008, background_material=materials.Material(epsilon=epsilon, mu=mu))
    solver.set_boundary(material=materials.PMC)
    solver.mesh(resolution=32)
    mode = solver.solve(max_refinements=0, num_modes=1, neff_guess=expected)[0]
...
def test_passive_loss_has_negative_neff_imaginary_part_for_public_convention() -> None:
    ...
    solver = ModeSolver1D(frequency=18000000000.0, x_range=0.01, background_material=materials.Material(epsilon=epsilon, mu=1.0))
    solver.mesh(resolution=36)
    mode = solver.solve(max_refinements=0, num_modes=1, neff_guess=expected)[0]
...
def test_pml_mode_solve_falls_back_cleanly_if_dense_ggev_is_unstable() -> None:
    solver = ModeSolver1D(frequency=c, x_range=(-2.0, 2.0))
    solver.add_layer(x_range=(-0.25, 0.25), material=materials.Material(epsilon=4.0, mu=1.0))
    solver.add_pml(thickness=0.6, sigma_max=4.0)
    solver.mesh(resolution=80)
    modes = solver.solve(max_refinements=0, residual_tolerance=1e-07, num_modes=2, neff_guess=1.8)
...
def test_active_constitutive_sign_is_rejected_with_convention_hint() -> None:
    with pytest.raises(ConfigurationError, match="non-positive imaginary"):
        ModeSolver1D(frequency=10000000000.0, x_range=0.005, background_material=materials.Material(epsilon=2.0 + 0.01j, mu=1.0))


def test_impedance_geometry_is_explicitly_rejected_until_robin_form_exists() -> None:
    solver = ModeSolver1D(frequency=10000000000.0, x_range=0.01)
    solver.add_geometry(shape=shapes.Interval(bounds=(0.004, 0.006)), material=SurfaceImpedance(impedance=50.0))
    solver.mesh(resolution=40)
