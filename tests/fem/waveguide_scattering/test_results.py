...
def make_result() -> ScatteringResult:
    ...
    return ScatteringResult(
        ...
        port_betas={key: complex(value).conjugate() for key, value in ({("left", 0): 2.0, ("right", 0): 2.0}).items()},
    )
...
def test_accepts_negative_imaginary_right_decaying_beta_with_roundoff() -> None:
    beta = -1e-12 - 2.0j
    result = replace(
        make_result(),
...
def test_s_power_check_excludes_evanescent_output_amplitudes() -> None:
    result = replace(
        ...
        port_betas={key: complex(value).conjugate() for key, value in ({
            ("left", 0): 2.0,
            ("left", 1): 1.0j,
            ("right", 0): 2.0,
        }).items()},
...
def test_plot_field_returns_axes_for_sampled_magnetic_field() -> None:
    ...
    axes = result.plot(component="Hz", quantity="abs").axes[0]
...
def test_deembedding_uses_negative_spatial_phase_convention() -> None:
