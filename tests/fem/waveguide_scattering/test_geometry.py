...
def test_background_pec_sheet_and_actual_slot_profiles_are_distinct() -> None:
    ...
    ground = geometry.add_pec(background=True, name='ground', x=-0.25, z='all')
    first = geometry.add_slot(name='main_slot', pec=ground, z=(-0.6, 0.6))
    second = geometry.add_slot(name='side_slot', pec='ground', z=(1.0, 1.4))
...
def test_background_pec_sheet_must_be_z_invariant() -> None:
    ...
    with pytest.raises(ConfigurationError, match="z-invariant"):
        geometry.add_pec(background=True, x=0.0, z=(-1.0, 1.0))
...
def test_pec_slot_must_be_compact_disjoint_and_on_background_sheet() -> None:
    ...
    geometry.add_slot(name='first', pec=ground, z=(-0.5, 0.5))
    ...
    finite = geometry.add_pec(background=False, name='actual_only', x=0.5, z=(-1.0, 1.0))
...
def test_internal_pec_cannot_duplicate_the_numerical_outer_boundary() -> None:
    ...
    with pytest.raises(ConfigurationError, match="strictly inside x_span"):
        geometry.add_pec(background=True, name='outer', x=-1.0, z='all')
