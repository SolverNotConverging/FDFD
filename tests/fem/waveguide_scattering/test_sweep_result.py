...
import fem_waveguide_scattering
...
def test_save_h5_uses_lazy_helper_and_passes_modes(
    ...
    fake_module = ModuleType("fem_waveguide_scattering.hdf5")
    ...
    monkeypatch.setitem(sys.modules, "fem_waveguide_scattering.hdf5", fake_module)
    monkeypatch.setattr(fem_waveguide_scattering, "hdf5", fake_module, raising=False)
    destination = Path("sweep.h5")
    sweep = make_sweep()

    written = sweep.save(destination)
