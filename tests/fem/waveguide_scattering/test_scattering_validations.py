...
def _solve_pec_guide(
    *,
    ...
    simulation = WaveguideScatteringSolver2D(frequency=299792458.0 / _WAVELENGTH, ky=ky, x_range=x_span, z_range=(-2.8e-06, 2.8e-06), background_material=materials.Material(epsilon=1.0, mu=1.0), boundary=materials.PEC)
    layer_x = simulation.x_span if layer_x is None else layer_x
    for index, (z_min, z_max, eps_r) in enumerate(layers):
        simulation.add_rectangle(x_range=layer_x, z_range=(z_min, z_max), name=f'layer_{index}', material=materials.Material(epsilon=eps_r, mu=1.0))
    if polygon is not None:
        simulation.add_polygon(points=polygon, name='compact_perturbation', material=materials.Material(epsilon=1.14, mu=1.0))
    simulation.add_pml(order=3, target_reflection=1e-08, thickness=7e-07, direction='z')
...
def test_reciprocal_transmission_matches_z_mirrored_left_incidence() -> None:
    # WaveguideScatteringSolver2D currently solves only left incidence.  Left incidence on
