...
def test_background_pec_sheet_and_slot_are_conforming_mesh_facets() -> None:
    ...
    ground = geometry.add_pec(background=True, name='ground', x=0.15, z='all')
    slot = geometry.add_slot(name='aperture', pec=ground, z=(-0.45, 0.35))
...
def test_finite_actual_only_pec_is_classified_as_inserted_facets() -> None:
    geometry = GeometryModel((-1.0, 1.0), (-2.0, 2.0), Material())
    ground = geometry.add_pec(background=True, name='ground', x=-0.4, z='all')
    geometry.add_slot(name='aperture', pec=ground, z=(-0.25, 0.25))
    plate = geometry.add_pec(background=False, name='finite_plate', x=0.35, z=(-0.7, 0.6))
...
def test_multiple_named_pec_slots_have_disjoint_released_facet_mappings() -> None:
    ...
    ground = geometry.add_pec(background=True, name='ground', x=0.0, z='all')
    first = geometry.add_slot(name='first', pec=ground, z=(-1.2, -0.7))
    second = geometry.add_slot(name='second', pec=ground, z=(0.4, 1.1))
...
def test_pec_distance_field_refines_elements_near_actual_sheet() -> None:
    ...
    geometry.add_pec(background=True, name='ground', x=0.0, z='all')
...
def test_scattering_mesh_preserves_global_cap_and_adds_dielectric_target() -> None:
    simulation = WaveguideScatteringSolver2D(frequency=1000000000.0, x_range=(-0.3, 0.3), z_range=(-0.5, 0.5))
    simulation.add_rectangle(x_range=(0.0, 0.3), z_range=simulation.z_range, background=True, name='dielectric', material=materials.Material(epsilon=4.0, mu=1.0))

    mesh = simulation.mesh(wavelength_elements=8)

    assert mesh.info.requested_maximum_edge == pytest.approx(simulation.wavelength / 16.0)
    assert simulation._mesh_size == pytest.approx(simulation.wavelength / 64.0)
