...
from FEM_Mode_Solver.exceptions import GeometryError, MeshError
from FEM_Mode_Solver.geometry import (
    Circle,
    GeometryModel1D,
    GeometryModel2D,
    PMLSpec,
...
from FEM_Mode_Solver.meshing import discretize_2d
...
def test_1d_solver_facade_accepts_distinct_pml_sides() -> None:
    solver = ModeSolver1D(10e9, 1.0, 1)
    solver.add_pml(0.1, direction="left")
    solver.add_pml(0.2, direction="right")

    assert [pml.direction for pml in solver.geometry.pmls] == ["x-", "x+"]
    with pytest.raises(GeometryError, match="already covers"):
        solver.add_pml(0.1, direction="x-")
