...
def test_diagonal_pencil_returns_nearest_modes_and_physical_residuals(
    ...
    values, vectors, residuals, restarts = refined_shift_invert_arnoldi(
        matrix_a,
        matrix_b,
        sigma=1.2,
        num_modes=2,
...
def test_cython_and_numpy_backends_agree() -> None:
    ...
    numpy_result = refined_shift_invert_arnoldi(
        matrix_a, matrix_b, kernel_backend="numpy", **arguments
    )
    cython_result = refined_shift_invert_arnoldi(
        matrix_a, matrix_b, kernel_backend="cython", **arguments
    )

    np.testing.assert_allclose(cython_result[0], numpy_result[0], atol=1e-11)
    np.testing.assert_allclose(cython_result[2], numpy_result[2], atol=1e-11)
    assert cython_result[3] == numpy_result[3]


def test_legacy_module_reexports_shared_solver() -> None:
    from Periodic_Solver_3D.refined_shift_invert_arnoldi import (
        refined_shift_invert_arnoldi as legacy_solver,
    )

    assert legacy_solver is refined_shift_invert_arnoldi


def test_legacy_four_tuple_maps_zero_tolerance_to_machine_precision_floor() -> None:
    values, _vectors, residuals, restarts = refined_shift_invert_arnoldi(
        diags(np.arange(1.0, 9.0), format="csr"),
        eye(8, format="csr"),
        sigma=1.2,
        num_modes=2,
    ...
    np.testing.assert_allclose(values, [1.0, 2.0], atol=1e-10)
