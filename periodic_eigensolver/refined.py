...
def refined_shift_invert_arnoldi(
    ...
    """Legacy four-tuple facade for :func:`solve_generalized`."""
    resolved_kernel = resolve_kernel_backend(kernel_backend)
    public_backend: ResolvedBackend = (
        "cython" if resolved_kernel == "cython" else "python"
    )
    compatibility_tolerance = max(
        0.0 if tol is None else float(tol),
        1e-12,
    )
