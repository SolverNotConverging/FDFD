...
.. code-block:: console
   ...
   python -m pip install -e ./libraries/periodic_eigensolver
...
.. code-block:: console
   ...
   python libraries/periodic_eigensolver/scripts/build_release_wheel.py

It builds into a private staging directory, runs the native-extension contract check, and only then moves the wheel into ``libraries/periodic_eigensolver/dist``. A failed optional extension build can therefore support an ordinary source install but cannot produce a release artifact. ``scripts/verify_native_wheel.py DIST_WHEEL`` remains available as an independent upload-job check.
