...
class SolverMixin:
    ...
    def _finish_result(self, result):
        ...
        context.update({key: _context_value(getattr(self, key)) for key in
            ("outer_potential", "pml") if hasattr(self, key)})
        if hasattr(self, "_boundary_kind"):
            context["boundary"] = _context_value(self._boundary_kind)
        if hasattr(self, '_scene_context'):
            context.update(self._scene_context())
...
class ElectromagneticSolverMixin(SolverMixin):
    @property
    def wavelength(self):
