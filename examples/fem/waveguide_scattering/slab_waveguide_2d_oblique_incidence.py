...
def main() -> None:
    ...
    core = Material(name="silicon core", epsilon=3.45**2)
    perturbed_core = Material(name="perturbed silicon", epsilon=3.451**2)
    simulation = scattering.WaveguideScatteringSolver2D(frequency=frequency_hz, angle=np.degrees(np.arcsin(0.1)), x_range=(-1.5e-06, 1.5e-06), z_range=(-2.5e-06, 2.5e-06), background_material=cladding)
    simulation.add_rectangle(x_range=(-2.2e-07, 2.2e-07), z_range=simulation.z_range, background=True, name='core', material=core)
    simulation.add_rectangle(x_range=(-2.2e-07, 2.2e-07), z_range=(-2.5e-07, 2.5e-07), name='perturbation', material=perturbed_core)
