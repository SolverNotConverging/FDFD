"""Command-line inspection and native-GUI launch for WaveFEM HDF5 files."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="wavefem-inspect-h5",
        description="Inspect a WaveFEM HDF5 result or open it in the native viewer.",
    )
    parser.add_argument(
        "path",
        nargs="?",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    path = arguments.path if arguments.path is not None else Path("wavefem_result.h5")
    saved = load_h5(path)
