...
from FEM_Periodic_Solver.meshing_3d import discretize_3d
import FEM_Periodic_Solver.solver_3d as solver_3d_module
from FEM_Periodic_Solver.solver_3d import _dense_candidates
...
def test_3d_refine_and_geometry_invalidation() -> None:
    solver = PeriodicModeSolver3D(
        20e9, (0.0, 4e-3), (0.0, 3e-3), (0.0, 2e-3), num_modes=1
    )
    coarse = solver.discretize(max_element_size=2e-3, wavelength_elements=4)
...
def test_rectangular_guide_te10_and_extruded_2d_agreement(
    ...
    solver_2d = PeriodicModeSolver2D(
        FREQUENCY,
        (0.0, WIDTH),
        (0.0, PERIOD),
        num_modes=1,
...
def test_3d_visualize_gui_saves_and_launches_all_modes(
    ...
    assert guide.visualize_with_gui() is marker
...
def test_3d_visualize_creates_matplotlib_figure(guide, guide_modes) -> None:
    ...
    assert "Mode 1" in axes.get_title()
