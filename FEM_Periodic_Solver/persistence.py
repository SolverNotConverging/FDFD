...
SCHEMA_FORMAT = "fem-periodic-modes"
SCHEMA_MAJOR = 1
SCHEMA_MINOR = 0
...
def _write_archive(path: Path, mode_sets: Sequence[PeriodicModeSet]) -> None:
    ...
    with h5py.File(path, "x", libver=("v110", "v110"), track_order=True) as archive:
        ...
        archive.attrs["time_convention"] = "exp(+j*omega*t)"
        archive.attrs["field_representation"] = "periodic-envelope"
        archive.attrs["length_unit"] = "m"
        archive.attrs["frequency_unit"] = "Hz"
        archive.attrs["producer"] = "FEM_Periodic_Solver"
        archive.attrs["producer_version"] = "0.1.0"
...
class PeriodicH5Archive:
    ...
    def __init__(self, path: str | os.PathLike[str]) -> None:
        ...
        with h5py.File(self.path, "r") as archive:
            ...
            if _text(archive.attrs.get("time_convention", "")) != "exp(+j*omega*t)":
...
def _viewer_candidates(executable_name: str) -> tuple[Path, ...]:
    ...
    configured = os.environ.get("FEM_PERIODIC_VIEWER_EXECUTABLE")
    if configured:
        candidates.append(Path(configured).expanduser())
    source_root = Path(__file__).resolve().parent.parent / "FEMPeriodicViewer"
    build_roots = [
        source_root / "build",
        *sorted(source_root.glob("build*")),
        *sorted(source_root.parent.glob("build*")),
    ]
    configurations = ("Release", "RelWithDebInfo", "Debug", "MinSizeRel")
    for build_root in build_roots:
        for binary_root in (build_root, build_root / "FEMPeriodicViewer"):
            candidates.append(binary_root / executable_name)
            candidates.append(
                binary_root
                / "fem-periodic-viewer.app"
                / "Contents"
                / "MacOS"
                / "fem-periodic-viewer"
            )
    ...
    located = shutil.which(executable_name) or shutil.which("fem-periodic-viewer")
    if located:
...
def launch_viewer(
    ...
    *,
    ...
    executable_name = "fem-periodic-viewer.exe" if os.name == "nt" else "fem-periodic-viewer"
    ...
    if executable is None:
        raise PersistenceError(
            "fem-periodic-viewer was not found. Install it on PATH or set "
            "FEM_PERIODIC_VIEWER_EXECUTABLE."
        )
