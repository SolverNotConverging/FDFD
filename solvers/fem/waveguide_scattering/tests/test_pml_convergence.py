...
def _solve(pml_thickness: float, maximum_edge: float) -> complex:
    ...
    simulation = WaveguideScatteringSolver2D(frequency=299792458.0 / _WAVELENGTH, ky=0.0, x_range=(0.0, 1e-06), z_range=(-outer, outer), background_epsilon=1.0, transverse_boundary='pec')
    simulation.add_rectangle(x_range=(0.0, 1e-06), z_range=(-_SLAB_HALF_WIDTH, _SLAB_HALF_WIDTH), epsilon=1.0 + _DELTA_EPS)
