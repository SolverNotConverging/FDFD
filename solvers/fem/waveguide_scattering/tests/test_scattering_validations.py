...
def _solve_pec_guide(
    *,
    ...
    simulation = WaveguideScatteringSolver2D(frequency=299792458.0 / _WAVELENGTH, ky=ky, x_range=x_span, z_range=(-2.8e-06, 2.8e-06), background_epsilon=1.0, transverse_boundary='pec')
    layer_x = simulation.x_span if layer_x is None else layer_x
    for index, (z_min, z_max, eps_r) in enumerate(layers):
        simulation.add_rectangle(x_range=layer_x, z_range=(z_min, z_max), epsilon=eps_r, name=f'layer_{index}')
    if polygon is not None:
        simulation.add_polygon(points=polygon, epsilon=1.14, name='compact_perturbation')
