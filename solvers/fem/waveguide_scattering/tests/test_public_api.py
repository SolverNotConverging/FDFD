...
def test_scattering_rejects_unimplemented_pmc_instead_of_imposing_pec() -> None:
    with pytest.raises(NotImplementedError, match="PMC"):
        wf.WaveguideScatteringSolver2D(frequency=299792458.0 / 1.55e-06, ky=0.0, x_range=(0.0, 1e-06), z_range=(-1e-06, 1e-06), transverse_boundary='pmc')
...
def test_obsolete_run_workflow_and_implicit_saving_are_unavailable() -> None:
    simulation = wf.WaveguideScatteringSolver2D(frequency=193400000000000.0, ky=0.0, x_range=(0.0, 1e-06), z_range=(-1e-06, 1e-06), transverse_boundary='pec')
...
def test_integrated_solver_rejects_lossy_uniform_leads() -> None:
    simulation = wf.WaveguideScatteringSolver2D(frequency=299792458.0 / 1.55e-06, ky=0.0, x_range=(0.0, 1e-06), z_range=(-2e-06, 2e-06), background_epsilon=1.0 - 0.01j, transverse_boundary='pec')
...
def test_integrated_solver_rejects_evanescent_incident_mode() -> None:
    simulation = wf.WaveguideScatteringSolver2D(frequency=299792458.0 / 1.55e-06, ky=0.0, x_range=(0.0, 2e-07), z_range=(-2e-06, 2e-06), background_epsilon=1.0, transverse_boundary='pec')
...
def test_open_integrated_modes_reject_unbound_pml_box_modes() -> None:
    simulation = wf.WaveguideScatteringSolver2D(frequency=299792458.0 / 1.0, ky=0.0, x_range=(-0.5, 0.5), z_range=(-1.0, 1.0), background_epsilon=2.25)
...
def test_high_level_solver_rejects_active_material_but_material_object_allows_it() -> None:
    ...
    with pytest.raises(wf.ConfigurationError, match="passive materials only"):
        wf.WaveguideScatteringSolver2D(frequency=299792458.0 / 1.55e-06, ky=0.0, x_range=(0.0, 1e-06), z_range=(-1e-06, 1e-06), background_epsilon=active.eps_r, transverse_boundary='pec')
...
def test_resolving_modes_clears_incident_and_rejects_stale_mode_object() -> None:
    simulation = wf.WaveguideScatteringSolver2D(frequency=299792458.0 / 1.0, ky=0.0, x_range=(0.0, 1.0), z_range=(-2.0, 2.0), transverse_boundary='pec')
...
def test_callback_device_accepts_explicit_compatible_mode_set() -> None:
    simulation = wf.WaveguideScatteringSolver2D.from_material_function(frequency=299792458.0 / 1.0, ky=0.0, x_range=((0.0, 1.0), (-2.0, 2.0))[0], z_range=((0.0, 1.0), (-2.0, 2.0))[1], epsilon=lambda x, z: 1.0 + 0.02 * (np.abs(z) < 0.2), background_epsilon=lambda x: np.ones_like(x), transverse_boundary='pec')
