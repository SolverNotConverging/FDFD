...
def _solve_open_slab(*, perturbed: bool):
    wavelength = 1.55e-6
    simulation = WaveguideScatteringSolver2D(frequency=299792458.0 / wavelength, angle=np.degrees(np.arcsin(0.1)), x_range=(-1.5e-06, 1.5e-06), z_range=(-2.5e-06, 2.5e-06), background_epsilon=1.44 ** 2)
    simulation.add_rectangle(x_range=(-2.2e-07, 2.2e-07), z_range='all', epsilon=3.45 ** 2, background=True, name='core')
    if perturbed:
        simulation.add_rectangle(x_range=(-2.2e-07, 2.2e-07), z_range=(-2.5e-07, 2.5e-07), epsilon=3.46 ** 2, name='perturbation')
