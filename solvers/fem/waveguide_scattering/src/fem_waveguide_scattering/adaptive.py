...
def solve_scattering(simulation, max_refinements, adaptive_tolerance):
    controls = simulation._solver_options
    ...
    if simulation.mesh_data is None:
        simulation.mesh(**getattr(simulation, "_mesh_settings", {"wavelength_elements": 4}))
    ...
    def estimate(result):
        ...
        def fields(basis, coefficient):
            ...
            magnetic = modified_curl(et, ey, simulation.ky * length) / (-1j * simulation._frequency.k0 * length * mu)
