...
class WaveguideScatteringSolver2D(ElectromagneticSolverMixin):
    ...
    def __init__(
        ...
        *,
        ...
        exterior = Material(eps_r=background_epsilon, mu_r=background_mu)
    ...
    def from_material_function(
        ...
        *,
        ...
        if not callable(epsilon) or not callable(background_epsilon):
            raise ConfigurationError("epsilon and background_epsilon must be callable.")
        simulation = cls(
            frequency=frequency,
            angle=angle,
            ky=ky,
    ...
    def _clone_at_frequency(self, frequency_hz: float) -> "WaveguideScatteringSolver2D":
    ...
    def add_rectangle(
        ...
        *,
        ...
        z_range: tuple[float, float] | Literal["all"],
        ...
        material = Material(eps_r=epsilon, mu_r=mu)
    ...
    def add_pml(self, *, thickness: float, direction: str = "all", order: int = 3,
                target_reflection: float = 1e-8) -> None:
        """Add matched layers on both ends of the selected x/z axis."""
        if direction not in ("x", "z", "all"):
            raise ConfigurationError("direction must be 'x', 'z', or 'all'.")
        layer = PML(thickness, order=order, target_reflection=target_reflection)
        candidate = PMLLayout(x=layer if direction in ("x", "all") else self.pml.x,
                              z=layer if direction in ("z", "all") else self.pml.z)
        candidate.validate_domain(self.x_span, self.z_span)
        self.pml = candidate
        self._invalidate()
    ...
    def _physical_material(
        ...
        if np.any(np.imag(eps) > 1e-14):
            raise ConfigurationError(
                "Integrated WaveguideScatteringSolver2D material callbacks must be passive."
            )
        return eps, mu
    ...
    def _transformed_material(
        ...
        sx, sz = self.pml.stretching(
            ...
            x_span=self.x_span,
            z_span=self.z_span,
            k_reference=self._frequency.k0,
        )
    ...
    def mesh(
        ...
        *,
        ...
        controls = replace(self._solver_options, element_order=element_order, quadrature_order=quadrature_order)
        ...
        derived = self._frequency.wavelength / (wavelength_elements * sizing_index)
        ...
        if self.angle is not None:
            self.ky = 0.0
        self._mesh_settings = dict(max_element_size=max_element_size, wavelength_elements=wavelength_elements,
                                   element_order=element_order, quadrature_order=quadrature_order,
    ...
    def _is_bound_guided_mode(self, mode: Mode) -> bool:
        ...
        eta = self.ky / self._frequency.k0
    ...
    def set_modes(self, modes: ModeSet) -> ModeSet:
        ...
        :meth:`from_material_function`; FEM Waveguide Scattering never guesses a discontinuous
        ...
        for mode in modes:
            ...
            if not np.allclose(
                ...
                raise ConfigurationError(
                    "Injected mode cross-section does not match WaveguideScatteringSolver2D.x_span."
                )
    ...
    def _resolve_modes_for_angle(self, selected_index: int) -> int:
        ...
        if self._angle_mode_request is None or self.modes is None:
            raise ConfigurationError(
                ...
                "WaveguideScatteringSolver2D.solve_modes() before selecting the incident mode."
            )
        if self._material_background is not None:
            raise ConfigurationError(
        ...
        self.ky = float(
            self._frequency.k0 * total_neff * sin(angle_radians)
        )
        expected_z_neff = seed.neff * cos(angle_radians)
        solver = ModeSolver(
            self._cross_section(),
            omega=self._frequency.omega,
            ky=self.ky,
    ...
    def solve_modes(
        ...
        *,
        ...
        if num_elements is None:
            h = self._mesh_size or self._frequency.wavelength / (10 * max(self._maximum_index(), 1.0))
    ...
    def set_incident_mode(
        ...
        *,
        ...
        if isinstance(mode, int):
            ...
            if self.modes is None:
                raise ConfigurationError(
                    "solve_modes must be called before binding an incident Mode to WaveguideScatteringSolver2D."
                )
    ...
    def sweep(
        ...
        *,
        linear_solver: Literal["direct"] = "direct",
        linear_solver_tolerance: float = 1e-10,
        projection_condition_limit: float = 1e12,
        max_refinements: int = 2,
        adaptive_tolerance: float = 0.05,
        ...
        scattered-field solve. Save the returned result explicitly with save(path).
        ...
        try:
            mesh_kwargs = dict(self._mesh_settings if mesh_options is None and hasattr(self, "_mesh_settings") else mesh_options or {})
            mode_kwargs = dict(mode_options or {})
            mode_kwargs.setdefault("max_refinements", max_refinements)
            mode_kwargs.setdefault("adaptive_tolerance", adaptive_tolerance)
        ...
        for value in frequencies:
            ...
            results.append(point.solve(linear_solver=linear_solver, linear_solver_tolerance=linear_solver_tolerance,
                projection_condition_limit=projection_condition_limit, max_refinements=max_refinements, adaptive_tolerance=adaptive_tolerance))
    ...
    def solve(self, *, linear_solver: Literal["direct"] = "direct",
              linear_solver_tolerance: float = 1e-10, projection_condition_limit: float = 1e12,
              max_refinements: int = 2, adaptive_tolerance: float = 0.05) -> ScatteringResult:
        """Solve without saving or opening a viewer."""
        from .adaptive import solve_scattering
        self._solver_options = replace(self._solver_options, linear_solver=linear_solver,
            tolerance=linear_solver_tolerance, projection_condition_limit=projection_condition_limit,
            max_refinements=max_refinements, adaptive_tolerance=adaptive_tolerance)
        return self._finish_result(solve_scattering(self, max_refinements, adaptive_tolerance))
    ...
    def _solve_once(
        ...
        length_scale = 1.0 / self._frequency.k0
        parameters = MaxwellParameters(
            k0=self._frequency.k0,
            ky=self.ky,
        ...
        system = assemble_mixed_system(
            self.mesh_data.mesh,
            parameters,
            intorder=self._solver_options.quadrature_order,
        ...
        H_sc_grid = curl_sc / (-1j * self._frequency.omega * MU_0 * mu_diagonal)
        ...
        density = -0.5 * self._frequency.omega * (
        ...
        result = ScatteringResult(
            ...
            frequency_hz=self._frequency.frequency,
            ky=self.ky,
            modes=forward_modes,
    ...
    def omega(self) -> float:
        return self._frequency.omega

    @property
    def k0(self) -> float:
        return self._frequency.k0
...
__all__ = ["WaveguideScatteringSolver2D", "_SolverOptions"]
