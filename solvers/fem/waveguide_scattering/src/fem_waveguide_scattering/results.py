"""fdfd_scattering results, sampled fields, S-parameters, and diagnostics."""
...
@dataclass(frozen=True, slots=True)
class ScatteringResult(ResultMixin):
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isinstance(mode, Mode) for mode in modes):
            raise ValueError("modes must contain only fem_waveguide_scattering.Mode objects.")
    ...
    @property
    def metadata(self):
        return MappingProxyType({"time_convention": "exp(+i*omega*t)",
            "field_representation": "exp(-i*ky*y)", "ky": self.ky})
        ...
        if len(input_modes) == 1:
            ...
            if beta_in is not None and self._is_effectively_propagating(beta_in):
                for side, reported_ratio, label in (
                    ...
                    # non-orthogonal projection Gram in WaveguideScatteringSolver2D.
                    if len(amplitudes) != 1:
                        continue
                    output_betas = [
