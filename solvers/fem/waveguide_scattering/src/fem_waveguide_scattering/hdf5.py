"""Versioned HDF5 persistence for FEM Waveguide Scattering scattering data.
...
the exact FEM Waveguide Scattering class version that produced them.  Each result may include
...
class H5ResultData:
    ...
    ``scene`` is a validated :class:`~fem_waveguide_scattering.scene.Scene2D` when the file
...
def _require_h5py() -> Any:
    try:
        ...
        raise ConfigurationError(
            ...
            "Install h5py in the active FEM Waveguide Scattering environment and ensure its "
...
def _json_ready(value: object) -> object:
    ...
    if isinstance(value, float):
        if np.isfinite(value):
            return value
        return {"__cem_type__": "float", "value": repr(value)}
    if isinstance(value, complex):
        return {
            "__cem_type__": "complex",
            "real": _json_ready(float(value.real)),
            "imag": _json_ready(float(value.imag)),
        }
    if isinstance(value, Path):
        return {"__cem_type__": "path", "value": str(value)}
    if isinstance(value, bytes):
        return {"__cem_type__": "bytes", "hex": value.hex()}
    if isinstance(value, np.ndarray):
        return {
            "__cem_type__": "ndarray",
    ...
    if isinstance(value, Mapping):
        return {
            "__cem_type__": "mapping",
            "items": [
                [_json_ready(key), _json_ready(item)] for key, item in value.items()
            ],
        }
    if isinstance(value, tuple):
        return {"__cem_type__": "tuple", "items": [_json_ready(item) for item in value]}
...
def _json_restore(value: object) -> object:
    ...
    if not isinstance(value, dict) or "__cem_type__" not in value:
        return value
    kind = value["__cem_type__"]
    if kind == "complex":
        return complex(
...
def _json_load(value: object, name: str) -> object:
    ...
    try:
        ...
        raise ValueError(f"{name} contains invalid FEM Waveguide Scattering metadata JSON.") from exc
...
def _write_scene(group: Any, scene: Scene2D) -> None:
    group.attrs["format"] = "fem_waveguide_scattering-scene"
...
def _write_file(
    ...
    *,
    ...
    write_envelope(handle, family="waveguide_scattering", kind=kind, dimension=2,
                   representation="sampled-fields; exp(-i*ky*y)")
...
def save_result_h5(
    ...
    *,
    ...
    """Save one scattering result using FEM Waveguide Scattering HDF5 schema version 1.
...
def _load_scene(group: Any) -> Scene2D:
    if _attribute_text(group.attrs.get("format"), f"{group.name}.format") != "fem_waveguide_scattering-scene":
...
def load_h5(path: os.PathLike[str] | str) -> H5FileData:
    """Load and validate one FEM Waveguide Scattering schema-v1 HDF5 file."""
    ...
    try:
        with h5py.File(source, "r") as handle:
            ...
            if format_name != SCHEMA_NAME:
                raise ValueError(
                    f"Not a FEM Waveguide Scattering HDF5 file: format={format_name!r}."
                )
            try:
                version = integer_index(handle.attrs["schema_version"])
            except (KeyError, TypeError) as exc:
                raise ValueError("FEM Waveguide Scattering HDF5 schema_version is missing or invalid.") from exc
            if version != SCHEMA_VERSION:
                raise ValueError(
                    f"Unsupported FEM Waveguide Scattering HDF5 schema version {version}; "
            ...
            if kind_text not in ("single", "sweep"):
                raise ValueError(f"FEM Waveguide Scattering HDF5 kind {kind_text!r} is invalid.")
            kind: Literal["single", "sweep"] = kind_text
            try:
                result_count = integer_index(handle.attrs["result_count"])
            except (KeyError, TypeError) as exc:
                raise ValueError("FEM Waveguide Scattering HDF5 result_count is missing or invalid.") from exc
            if result_count <= 0 or (kind == "single" and result_count != 1):
                raise ValueError("FEM Waveguide Scattering HDF5 result_count is inconsistent with kind.")
