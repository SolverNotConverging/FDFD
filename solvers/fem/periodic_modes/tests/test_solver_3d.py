...
from fem_periodic_modes import Box, PeriodicModeSolver2D, PeriodicModeSolver3D, Cylinder, Material, SolverError, Sphere
...
def test_invalid_3d_integer_controls_raise_configuration_error(invalid):
    from fem_periodic_modes import ConfigurationError
...
def guide() -> PeriodicModeSolver3D:
    solver = PeriodicModeSolver3D(frequency=FREQUENCY, x_range=(0.0, WIDTH), y_range=(0.0, HEIGHT), z_range=(0.0, PERIOD), background_epsilon=EPSILON)
...
@pytest.mark.parametrize(
    ("shape", "kind"),
    (
        (Cylinder((0.32, 0.32), 0.12, (0.2, 0.8)), "pec"),
        (Sphere((0.68, 0.68, 0.5), 0.14), "pmc"),
    ),
)
...
def test_3d_refine_and_geometry_invalidation() -> None:
    ...
    solver.add_box(epsilon=2.0, mu=1.0, x_range=(0.001, 0.002), y_range=(0.001, 0.002), z_range=(0.0005, 0.0015))
...
def test_rectangular_guide_te10_and_extruded_2d_agreement(
    ...
    solver_2d = PeriodicModeSolver2D(frequency=FREQUENCY, x_range=(0.0, WIDTH), z_range=(0.0, PERIOD), polarization='TE', background_epsilon=EPSILON)
