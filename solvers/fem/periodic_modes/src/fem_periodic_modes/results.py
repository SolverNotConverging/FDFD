    ...
    def __init__(
        ...
        *,
        ...
        object.__setattr__(self, "metadata", MappingProxyType({**({} if metadata is None else {str(k): _freeze(v) for k, v in metadata.items()}),
            "time_convention": "exp(+i*omega*t)", "field_representation": "periodic-envelope"}))
...
@dataclass(frozen=True, slots=True, init=False, eq=False)
class PeriodicModeSet(ResultMixin, Sequence[PeriodicMode]):
    ...
    def mode(self, number: int) -> PeriodicMode:
        if isinstance(number, (bool, np.bool_)) or not isinstance(number, (int, np.integer)) or not 0 <= number < len(self):
            raise IndexError(f"mode must be an integer in 0..{len(self)-1}.")
