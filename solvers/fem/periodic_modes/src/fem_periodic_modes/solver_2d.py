...
class PeriodicModeSolver2D(ElectromagneticSolverMixin):
    ...
    def add_pml(self, *, thickness: float, order: int=3, sigma_max: float=5.0, direction: str='x') -> PMLSpec:
        normalized = str(direction).strip().lower()
        if normalized == "both":
            normalized = "x"
        return self.geometry.add_pml(
            PMLSpec(
                _positive_real(thickness, "thickness"),
    ...
    def _mesh_impl(
        ...
        *,
        ...
        if resolution is not None:
            ...
            maximum = min((self.x_span[1] - self.x_span[0]) / (nx - 1), self.period / (nz - 1))
            ...
            maximum = min(wavelength / wavelength_count, max(self.x_span[1] - self.x_span[0], self.period) / 4.0)
    ...
    def _require_current_systems(self) -> dict[str, PeriodicFEMSystem2D]:
        if self._mesh_data is None or not self._systems:
            raise NotDiscretizedError("Call mesh() before solve().")
        if self._discretized_revision != self.geometry.revision:
            raise StaleDiscretizationError("Geometry changed; call mesh() again.")
    ...
    def solve(self, *, neff_guess: complex | None=None, num_modes: int=4, direction: Direction='forward', eigensolver: str="auto", arnoldi_backend: str="auto", eigensolver_tolerance: float=1e-10, residual_tolerance: float=1e-08, propagation_ratio_tolerance: float=0.001, max_pml_fraction: float | None=0.5, dense_linearization_limit: int=700, max_refinements: int=2, adaptive_tolerance: float=0.05):
        """Solve and return modes with separate algebraic and adaptive controls."""
        from .adaptive import solve_periodic
        options = {'neff_guess': neff_guess, 'num_modes': num_modes, 'direction': direction, 'eigensolver': eigensolver, 'arnoldi_backend': arnoldi_backend, 'eigensolver_tolerance': eigensolver_tolerance, 'residual_tolerance': residual_tolerance, 'propagation_ratio_tolerance': propagation_ratio_tolerance, 'max_pml_fraction': max_pml_fraction, 'dense_linearization_limit': dense_linearization_limit}
        return self._finish_result(solve_periodic(self, 2, (), options, max_refinements, adaptive_tolerance))
        ...
        *,
        ...
        if max_pml_fraction is None:
            ...
            if not np.isfinite(pml_limit) or not 0.0 <= pml_limit <= 1.0:
                raise ConfigurationError("max_pml_fraction must lie in [0, 1] or be None.")
        target = self._default_guess() if neff_guess is None else _finite_complex(
    ...
    def mesh(self, *, max_element_size: float | None=None, resolution: tuple[int, int] | None=None, wavelength_elements: int=4, element_order: int=1, quadrature_order: int=4):
        """Build the initial mesh; solve() may subsequently refine it."""
        settings = {'max_element_size': max_element_size, 'resolution': resolution, 'wavelength_elements': wavelength_elements, 'element_order': element_order, 'quadrature_order': quadrature_order}
