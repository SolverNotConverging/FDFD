...
from fem_electrostatics import ElectrostaticSolver, Interval, Rectangle, SolverError
...
def test_default_adaptation_reduces_poisson_field_error(dimension):
    ...
    solver.set_potential(region='right', potential=0.0)
    shape = Interval((0.0, 1.0)) if dimension == 1 else Rectangle((0.0, 1.0), (0.0, 0.5))
    solver.add_charge_density(region=shape, density=EPSILON_0)
...
def test_exact_dielectric_flux_does_not_trigger_refinement():
    ...
    solver.add_object(region=Interval((0.5, 1.0)), epsilon=4.0)
    solver.set_potential(region='left', potential=0.0)
...
def test_element_budget_preserves_last_usable_mesh():
    ...
    solver.add_charge_density(region=Interval((0.0, 1.0)), density=EPSILON_0)
