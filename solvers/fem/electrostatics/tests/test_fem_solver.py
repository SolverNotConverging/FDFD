...
from fem_electrostatics import Circle, ElectrostaticSolver, Interval
...
def test_1d_linear_dirichlet_solution_and_field() -> None:
    ...
    solver.set_potential(region='right', potential=10.0, name='hot')
...
def test_1d_dielectric_interface_preserves_displacement() -> None:
    solver = ElectrostaticSolver(dim=1, outer_potential=None, x_range=(0.0, 1.0))
    solver.add_object(region=Interval((0.5, 1.0)), name='high_dk', epsilon=4.0)
    solver.set_potential(region='left', potential=0.0)
    solver.set_potential(region='right', potential=1.0)
    result = solver.solve()
...
def test_charge_density_drives_poisson_solution() -> None:
    ...
    solver.add_charge_density(region=Interval((0.0, 1.0)), density=1e-12)
    result = solver.solve()
...
def test_curved_internal_conductor_is_conforming_and_fixed() -> None:
    ...
    solver.set_potential(region=electrode, potential=1.0, name='round_electrode')
    result = solver.solve()
...
def test_geometry_change_invalidates_mesh_and_solution() -> None:
    ...
    solver.add_object(region=Interval((0.2, 0.4)), epsilon=3.0)
