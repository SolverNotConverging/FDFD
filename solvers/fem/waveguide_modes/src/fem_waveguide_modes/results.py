...
class SampledFields:
    ...
    def __init__(
        ...
        *,
        ...
        object.__setattr__(self, "metadata", _frozen_metadata({**(metadata or {}),
            "time_convention": "exp(+i*omega*t)", "field_representation": "exp(-i*beta*z)"}))
...
class Mode:
    ...
    def __post_init__(self) -> None:
        if isinstance(self.index, (bool, np.bool_)) or int(self.index) < 0:
            raise ValueError("index must be a nonnegative, zero-based mode index.")
...
@dataclass(frozen=True, slots=True, init=False, eq=False)
class ModeSet(ResultMixin, Sequence[Mode]):
    ...
    def mode(self, number: int) -> Mode:
        ...
        if isinstance(number, (bool, np.bool_)) or not isinstance(number, (int, np.integer)) or not 0 <= number < len(self):
            raise IndexError(f"mode must be an integer in 0..{len(self)-1}; got {number!r}.")
    ...
    def by_polarization(self, polarization: str) -> "ModeSet":
        ...
        object.__setattr__(result, "_mesh_snapshot", self.mesh_data)
        return result
