...
class ModeSolver2D(WaveguideScene2D, ElectromagneticSolverMixin):
    ...
    def add_mesh_refinement(self, *, shape: Shape2D, max_element_size: float, transition_width: float=0.0, name: str | None=None) -> MeshRefinement:
        ...
        :meth:`mesh`; it does not create a material or boundary object.
        """
    ...
    def add_pml(self, *, thickness: float, order: int=3, sigma_max: float=5.0, direction: str='all') -> PMLSpec:
        ...
        width = _positive_real(thickness, "thickness")
    ...
    def system(self) -> ModeFEMSystem2D:
        if self._system is None:
            raise NotDiscretizedError("Call mesh() before requesting the FEM system.")
        return self._system
    ...
    def _require_current_system(self) -> ModeFEMSystem2D:
        if self._system is None or self._mesh_data is None:
            raise NotDiscretizedError(
                "The continuous scene has not been discretized; call mesh() before solve()."
            )
        if self._discretized_revision != self.geometry.revision:
            raise StaleDiscretizationError(
                "The geometry changed after discretization; call mesh() again."
            )
        return self._system
    ...
    def solve(self, *, neff_guess: complex | None=None, num_modes: int=4, direction: Direction='forward', eigensolver_tolerance: float=1e-10, residual_tolerance: float=1e-08, divergence_tolerance: float=1e-07, propagation_ratio_tolerance: float=0.001, dense_linearization_limit: int=700, max_refinements: int=2, adaptive_tolerance: float=0.05):
        ...
        options = {'neff_guess': neff_guess, 'num_modes': num_modes, 'direction': direction, 'eigensolver_tolerance': eigensolver_tolerance, 'residual_tolerance': residual_tolerance, 'divergence_tolerance': divergence_tolerance, 'propagation_ratio_tolerance': propagation_ratio_tolerance, 'dense_linearization_limit': dense_linearization_limit}
        return self._finish_result(solve_2d(self, (), options, max_refinements, adaptive_tolerance))
    ...
    def mesh(self, *, max_element_size: float | None=None, resolution: tuple[int, int] | None=None, wavelength_elements: int=4, material_aware: bool=True, interface_refinement: float | None=0.7, interface_refinement_width: float | None=None, boundary_refinement: float | None=0.5, boundary_refinement_width: float | None=None, element_order: int=1, quadrature_order: int=4):
        """Build the initial mesh; solve() may subsequently refine it."""
        settings = {'max_element_size': max_element_size, 'resolution': resolution, 'wavelength_elements': wavelength_elements, 'material_aware': material_aware, 'interface_refinement': interface_refinement, 'interface_refinement_width': interface_refinement_width, 'boundary_refinement': boundary_refinement, 'boundary_refinement_width': boundary_refinement_width, 'element_order': element_order, 'quadrature_order': quadrature_order}
