"""Backend-neutral visualizers for :mod:`fem_waveguide_modes` results."""
...
def _select_mode(modes: tuple[Mode, ...], mode: int | Mode) -> tuple[Mode, int]:
    ...
    if isinstance(mode, Mode):
        return mode, modes.index(mode) if mode in modes else mode.index
    if isinstance(mode, (bool, np.bool_)):
        raise TypeError("mode must be a zero-based integer or a Mode instance.")
    number = int(mode)
    if not 0 <= number < len(modes):
        raise IndexError(f"mode must be between 0 and {len(modes)-1}; got {mode!r}.")
    return modes[number], number
...
def visualize(
    ...
    **legacy_component_flags: Any,
    ...
    ``mode`` is zero-based for compatibility with the original mode solvers.
...
class ModeViewer:
    ...
    @property
    def mode(self) -> Mode:
        return self.modes[self.mode_number]

    def _set_mode(self, label: str) -> None:
        self.mode_number = self._mode_labels.index(label)
        self._draw()
