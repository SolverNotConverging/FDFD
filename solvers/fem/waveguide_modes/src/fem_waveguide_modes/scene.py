"""Material-first geometry API for the waveguide FEM implementations."""
...
class WaveguideSceneMixin(SceneMixin):
    def _prepare_geometry(self, record):
        ...
        if outside:
            ...
            elif isinstance(record.material, materials.Material):
    ...
    def _insert_geometry(self, record):
        ...
        kind = material.kind if isinstance(material, materials.IdealBoundary) else 'impedance'
        impedance = None if kind != 'impedance' else material.at_frequency(frequency=self.frequency)
        if len(self._physical_axes) == 1:
            return self.geometry.add_boundary(shape, kind, impedance=impedance, name=record.name)
        return self.geometry.add_boundary(shape, kind, impedance=impedance, name=record.name, clip=record.clip)
...
class WaveguideScene1D(WaveguideSceneMixin):
    _physical_axes = ('x',)
...
class WaveguideScene2D(WaveguideSceneMixin):
    _physical_axes = ('x', 'y')
