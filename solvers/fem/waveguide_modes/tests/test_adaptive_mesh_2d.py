...
from fem_waveguide_modes.exceptions import MeshError
...
def test_solver_refine_remeshes_and_reassembles() -> None:
    solver = ModeSolver2D(frequency=1000000.0, x_range=1.0, y_range=1.0)
    solver.add_mesh_refinement(shape=Rectangle((0.35, 0.65), (0.35, 0.65)), max_element_size=0.06, name='center')
    coarse = solver.mesh(max_element_size=0.18, wavelength_elements=4, material_aware=False, boundary_refinement=None)
