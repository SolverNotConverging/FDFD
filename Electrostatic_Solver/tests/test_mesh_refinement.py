...
def test_high_dk_region_receives_smaller_gmsh_elements() -> None:
    solver = ElectrostaticSolver(dim=2, domain=((0.0, 2.0), (0.0, 1.0)))
    solver.add_object(Rectangle((1.0, 2.0), (0.0, 1.0)), erxx=16.0, name="high_dk")
    mesh = solver.discretize(
        max_element_size=0.16,
...
def test_dirichlet_boundary_refinement_reduces_near_boundary_size() -> None:
    ...
    solver.set_potential(Rectangle((0.45, 0.55), (0.2, 0.8)), 1.0, name="electrode")
    mesh = solver.discretize(
        max_element_size=0.14,
        material_aware=False,
        interface_refinement=None,
        boundary_refinement=0.35,
        boundary_refinement_width=0.12,
    )
    centers = mesh.nodes[mesh.elements].mean(axis=1)
    edge = _element_edge_mean(mesh.nodes, mesh.elements)
